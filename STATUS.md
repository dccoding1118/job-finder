...
- [ ] 公開 GitHub repo。多數資安與對外可見度設定被 private＋免費方案擋住，須依下列**硬順序**在轉 public 當天一次做完（Dependabot alerts 與 automated security fixes 已於 private 階段開啟）：
  ...
  5. 全綠合併 → 設 main 分支保護（required status checks 填 `check`、`analyze`；solo dev 不設 required reviews，會卡死自己）→ 打首版 tag 觸發 release。
...
- [ ] S2 Windows 自部署：以 Task Scheduler（登入時啟動 `serve`、每日觸發 `run`）取代 systemd user unit，設定與資料路徑改走 Windows 使用者目錄。`windows/amd64` 工件已由 release 流程產出。完成後自用不再需要 SSH 通道（該通道只是遠端後端的過渡作法）。
