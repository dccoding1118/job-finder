...
test("Side Panel loads, filters, shows a job, persists theme, updates apply state, and starts a run", async ({ page }) => {
  ...
  await page.addInitScript(() => {
    const jobs = [{ id: 2, source: "yourator", url: "https://www.yourator.co/jobs/2", title: "Synthetic job", company_name: "Example", salary_min: 100000, salary_max: 120000, location: "Taipei", score_total: 90, process_state: "letter_ready", apply_state: "pending", verdict: "recommended", letter_state: "ready" }];
    window.__calls = [];
  ...
  await page.getByRole("button", { name: "手動抓取自動來源" }).click();
