...
| Live 驗收 | Linux 與 Windows 測試環境各以 binary 內建的 `jobfinder verify live`，在實際安裝上驗證真實公開來源與已授權 CLI Runner：暫時安排出可驗證的情境，經常駐 worker 的單筆入口各驗一筆篩選、評分與求職信，跑完復原設定；至少取得一筆真資料並驗證格式，外部能力不足明確標為 `ENVIRONMENT_BLOCKED` |
