...
**求職信產製歷程**（PRD R8.2）：求職信卡片下方一個預設收合的區塊，展開時才送出 `GET /api/v1/jobs/{id}/letter-history`——一次產製帶著數版 draft 全文，不該在每次職缺載入時一併拉進來。第一層是每一次產製：結果（已過審／未產出信件／產生中）、開始時刻、輪數與兩個角色的 runner，新到舊排列，重跑不覆蓋舊的；`finalized` 的產製不在此標示結果，該版已達輪數上限由上方求職信卡片說明，同一畫面不重複第二次。展開某一次後依序為該次的審查摘要逐行、逐輪內容與該次產出的信件全文——逐輪內容為每一筆 drafter／reviewer 呼叫的輪次、角色、runner 與輸出，drafter 的輸出取契約 JSON 內的信件本文呈現，解析不出者呈現原始輸出，呼叫未通過契約者另標示。審查摘要的 `revise:`／`guard:`／`error:` 前綴在此譯為介面用語。升級前的產製沒有逐輪呼叫，該次呈現摘要、信件與說明原因。外層區塊與每一次產製各有自己的收合箭頭，方向只隨該層的展開狀態轉。**每次展開都重新讀取**：先以上次讀到的內容立即呈現，同一個請求的回應覆蓋它。展開期間新要求的產製、仍在跑的產製都只在新的一次讀取才看得到，沿用上次的結果會讓剛跑完的產製一直顯示為不存在。要求產製求職信後該區收起並丟棄上次的結果，下次展開重讀。展開狀態由 Side Panel 自己持有：職缺資料每次更新都重繪該區，交給 details 元素會在下一次重繪時闔上。
