...
**求職信產製歷程**（PRD R8.2）：求職信卡片下方一個預設收合的區塊，展開時才送出 `GET /api/v1/jobs/{id}/letter-history`——一次產製帶著數版 draft 全文，不該在每次職缺載入時一併拉進來。第一層是每一次產製：結果（已過審／已達輪數上限／未產出信件／產生中）、開始時刻、輪數與兩個角色的 runner，新到舊排列，重跑不覆蓋舊的。展開某一次後為該次的審查摘要逐行與逐輪內容——每一筆 drafter／reviewer 呼叫的輪次、角色、runner 與輸出全文，呼叫未通過契約者另標示。審查摘要的 `revise:`／`guard:`／`error:` 前綴在此譯為介面用語。升級前的產製沒有逐輪呼叫，該次只呈現摘要並說明原因。展開狀態由 Side Panel 自己持有：職缺資料每次更新都重繪該區，交給 details 元素會在下一次重繪時闔上。
