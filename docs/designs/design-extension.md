...
| 系統 | 依序呈現「進行中」「連線與設定」「Profile」「自動篩選與評分」「疑似重複」「自動與手動批次」「處理進度」「Agent 呼叫紀錄」「批次歷程」群組 | 檢視此刻正在跑的作業、開啟 Options、開始設定／編輯 Profile、更新過時評分、開關自動篩選與評分、查看每日 08:30 排程、手動抓取、檢視待處理量、裁決疑似重複職缺、檢視 Agent 呼叫結果與歷程 |
...
**求職信生成入口**（PRD R5.0、R6.8）：對照區依 API 回傳的 `letter_state` 決定呈現——`none` 顯示「產生求職信」按鈕；按下後送出 `POST /api/v1/jobs/{id}/letter`，立即轉為處理中並停用按鈕（回應為受理，不等待完成）；`requested` 顯示處理中與說明「後端完成後可重新載入」；`ready` 顯示求職信與複製，另提供「重新產製」——同一個 `POST /api/v1/jobs/{id}/letter`，送出後同樣立即轉為處理中，既有信件與其產製紀錄保留為歷史；`letters.status` 為 `finalized` 時另標示該版已達輪數上限、未經最後一次審查，建議投遞前自行過目；`failed` 顯示產製失敗與「再次產生」，失敗原因（Agent 呼叫失敗或最終版未通過內容防線）於系統頁的 Agent 呼叫紀錄可查。生成結果由使用者重新整理或下次載入時取得，插件不得為此輪詢高頻請求。

**求職信產製歷程**（PRD R8.2）：求職信卡片下方一個預設收合的區塊，展開時才送出 `GET /api/v1/jobs/{id}/letter-history`——一次產製帶著數版 draft 全文，不該在每次職缺載入時一併拉進來。第一層是每一次產製：結果（已過審／未產出信件／產生中）、開始時刻、輪數與兩個角色的 runner，新到舊排列，重跑不覆蓋舊的；`finalized` 的產製不在此標示結果，該版已達輪數上限由上方求職信卡片說明，同一畫面不重複第二次。展開某一次後為**逐輪**呈現，一輪一段、依序排列：輪次與該輪各角色的 runner、該輪產出的信件全文、該輪的結果。信件取該輪 drafter 呼叫的契約 JSON 內的本文，解析不出者呈現原始輸出。該輪結果取自審查摘要的對應行（每輪恰好一行）：`revise` 呈現為「修改建議」與逐條問題（逐條取自該輪 reviewer 呼叫輸出的 `issues`，取不到才退回摘要那一行的文字）、`guard` 為未通過保護規則、`error` 為呼叫失敗、`approve` 為「審核通過。」、`finalized` 為「輪數用完，直接定稿。」。呼叫未通過契約者在該輪的 runner 標示。逐輪之後以「這次產出的信件」收尾，呈現該次產製存下來的信件——它不一定等於最後一輪的草稿：過審的 reviewer 可以回傳編輯過的版本，存下來、被複製、被投遞的是那一版，因此與最後一輪的草稿不同時另標示「審查時經過編輯」。升級前的產製沒有逐輪呼叫，各輪說明原因，該次產出的信件照常呈現。外層區塊與每一次產製各有自己的收合箭頭，方向只隨該層的展開狀態轉。**每次展開都重新讀取**：先以上次讀到的內容立即呈現，同一個請求的回應覆蓋它。展開期間新要求的產製、仍在跑的產製都只在新的一次讀取才看得到，沿用上次的結果會讓剛跑完的產製一直顯示為不存在。要求產製求職信後該區收起並丟棄上次的結果，下次展開重讀。展開狀態由 Side Panel 自己持有：職缺資料每次更新都重繪該區，交給 details 元素會在下一次重繪時闔上。
...
**進行中**：系統頁最上方一區，合併兩個來源——`GET /api/v1/runs` 中 `state` 為 `running` 的抓取，與 `GET /api/v1/status` 的 `in_flight`。每列顯示作業名稱（抓取職缺／篩選／評分／產生求職信）、對象（職缺 ID，抓取則為已收筆數）與**已經跑了多久**。沒有任何作業時顯示空狀態，並說明作業開始後會在此顯示。這一區回答的是其他每一區都答不了的問題：待處理筆數、稽核列與批次統計全都在工作結束後才變動，一次跑數分鐘的呼叫期間它們完全靜止，正在工作與已經當掉因此無從分辨。

**批次歷程**：每輪顯示觸發方式、開始時刻、執行狀態、耗時、抓取事實與該輪職缺的現行判定分布。觸發方式、統計鍵與判定鍵一律以中文呈現，與 UI 其他處對同一概念的用語一致（每日排程／手動（指令列）／手動（側邊欄）；查詢數、抓取、新職缺、錯誤；推薦、不推薦、評分中、篩選中、待看、不適合）。執行狀態取自 API 導出的四態，`stalled` 另附一行說明該批次已超過門檻沒有回報進度、執行它的程序可能已結束。

處理進度、Agent 呼叫紀錄與每日用量只在重新整理或載入時取得，不輪詢。唯一的例外是**系統頁開啟且確實有作業在跑**時：每 5 秒重取 `GET /api/v1/status` 與 `GET /api/v1/runs`，直到沒有作業在跑或使用者離開該頁即自行停止。已耗時若不會自己前進，它就只是另一個看不出死活的靜止畫面；輪詢的條件同時保證閒置的面板一個請求都不發。
