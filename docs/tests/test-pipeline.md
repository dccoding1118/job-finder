...
| PT-98 | `max_letter_rounds` 設為不同值 | Drafter 與 Reviewer 的呼叫次數隨設定改變；letter 的 `rounds` 記實際跑過的輪數 |
...
| 編號 | 測試情境 | 預期結果 |
|---|---|---|
| PT-60 | 來源逐批交付兩筆職缺，`Progress` 每次讀取已入庫筆數 | 第一次回報時已入庫一筆、第二次兩筆；回報的累計數與實際入庫數一致 |
| PT-61 | `Progress` 回錯 | `Fetch` 回傳該錯誤且只回報一次；已寫入的職缺保留 |
| PT-62 | `Activity` 依序登記兩個工作後逐一結束 | `InFlight` 依起始時刻由舊至新回報；結束者離開；同一結束函式呼叫兩次無副作用 |
| PT-63 | 對 nil `Activity` 登記工作 | 不 panic、不記錄；`InFlight` 回空 |
