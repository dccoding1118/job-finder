...
| PT-31 | 達門檻的合成 Job、Score 與 `interview` 事件 | Calibrator 輸入只含 `preferences` 區段與樣本，不含 `summary`、經歷與成就；輸出建議套用於副本後產生 unified diff；`profile.yaml` 與 active snapshot 未變 |
| PT-32 | 成功樣本中含同一群組的多筆 alias | 只取 canonical 一筆計入樣本，重複刊登不重複加權 |
| PT-33 | 對照樣本（`ghosted`）不存在 | 仍可執行，僅以成功樣本作答；不因缺對照而失敗 |
| PT-34 | Calibrator 建議指向 `preferences` 以外的欄位 | 拒絕整份建議並回安全錯誤；不產生 diff、不寫入 Profile |
