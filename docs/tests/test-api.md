...
| AT-15A | 對 `letter_ready` Job 送出同一路由 | 同樣受理並轉為 `letter_requested`；既有 Letter 與其產製紀錄保留，不被刪除或改寫 |
| AT-16 | 對 `letter_requested` Job 重複送出 | 冪等回 `requested`；不重複呼叫 `RequestLetter`、不新增事件 |
| AT-17 | 對 `scored`、`discovered` 或不存在的 Job 送出 | 回 4xx；不呼叫 pipeline、DB 狀態不變 |
...
| AT-87 | GET runs 的執行狀態 | 已收尾且無錯誤回 `done`、已收尾且帶錯誤回 `failed`、未收尾且心跳在門檻內回 `running`、心跳逾期或不存在回 `stalled`；`heartbeat_at` 一併回傳 |
| AT-88 | GET status 的 `in_flight` | 有工作在跑時每筆含 `stage`、`job_id`、`started_at` 與不小於實際已耗時的 `elapsed_ms`；閒置時為空陣列而非缺欄位 |
